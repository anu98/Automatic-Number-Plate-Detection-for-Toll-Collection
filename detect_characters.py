...
def loadKNNDataAndTrainKNN():
    ...
    try:
        npaFlattenedImages = np.loadtxt("/Users/ayaskant/PycharmProjects/Number_Plate_Recognition/Training_data/flattened_images.txt", np.float32)                 # read in training images
    except:                                                                                 # if file could not be opened
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
        ...
        possiblePlate.imgGrayscale, possiblePlate.imgThresh = image_preprocess.preprocess(possiblePlate.imgPlate)     # preprocess to get grayscale and threshold images
...
def findPossibleCharsInPlate(imgGrayscale, imgThresh):
    ...
    for contour in contours:                        # for each contour
        possibleChar = total_characters_possible.PossibleChar(contour)

...
import total_plates_possible
import total_characters_possible
...
def detectPlatesInScene(imgOriginalScene):
    ...
    imgGrayscaleScene, imgThreshScene = image_preprocess.preprocess(imgOriginalScene)         # preprocess to get grayscale and threshold images
    ...
    listOfListsOfMatchingCharsInScene = detect_characters.findListOfListsOfMatchingChars(listOfPossibleCharsInScene)
...
def findPossibleCharsInScene(imgThresh):
    ...
    for i in range(0, len(contours)):                       # for each contour
        ...
        possibleChar = total_characters_possible.PossibleChar(contours[i])

        if detect_characters.checkIfPossibleChar(possibleChar):                   # if contour is a possible char, note this does not compare to other chars (yet) . . .
...
def extractPlate(imgOriginal, listOfMatchingChars):
    possiblePlate = total_plates_possible.PossiblePlate()           # this will be the return value
    ...
    fltHypotenuse = detect_characters.distanceBetweenChars(listOfMatchingChars[0], listOfMatchingChars[len(listOfMatchingChars) - 1])
